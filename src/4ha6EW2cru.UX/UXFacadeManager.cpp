...
namespace UX
{
	void UXFacadeManager::Initialize( ISystemComponent* scriptComponent )
	{
		...
		IScriptFacade* inputFacade = m_facadeFactory->CreateScriptFacade( IScriptFacade::INPUT, scriptComponent );
		m_facades.push_back( inputFacade );
		inputFacade->Initialize( );

		IScriptFacade* instFacade = m_facadeFactory->CreateScriptFacade( IScriptFacade::INSTRUMENTATION, scriptComponent );
		m_facades.push_back( instFacade );
		instFacade->Initialize( );

		IScriptFacade* netFacade = m_facadeFactory->CreateScriptFacade( IScriptFacade::NETWORK, scriptComponent );
