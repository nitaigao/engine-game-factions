...
def main():
    ...
    for project, classes in classListFromProject.items():
        ...
        if len(clist):
            ...
            #add to cvsignore
            try: oldtxt = open( os.path.join(folder,".cvsignore") ).read()
            except: oldtxt = ""
            newtxt = oldtxt
            if newtxt.find(headerName) == -1:
                print "Add",headerName, "to", folder
                newtxt += "\n" + headerName
            if newtxt.find(cxxName) == -1:
                print "Add",cxxName, "to", folder
                newtxt += "\n" + cxxName
            if newtxt != oldtxt:
                open( os.path.join(folder,".cvsignore"), "w" ).write( newtxt )
