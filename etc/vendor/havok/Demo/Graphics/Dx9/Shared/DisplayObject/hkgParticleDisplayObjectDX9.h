...
#ifndef HK_GRAPHICS_PARTICLE_DISPLAY_OBJECT_DX9__H
#define HK_GRAPHICS_PARTICLE_DISPLAY_OBJECT_DX9__H
...
class hkgVertexShaderDX9;
class hkgPixelShaderDX9;

class hkgParticleDisplayObjectDX9 : public hkgParticleDisplayObject
{
	public:

		static hkgParticleDisplayObject* HK_CALL createParticleDisplayObjectDX9( hkgDisplayContext* ctx );

		hkgParticleDisplayObjectDX9( hkgDisplayContext* ctx );

		virtual ~hkgParticleDisplayObjectDX9();
	...
	protected:
		...
		void renderParticles( hkgDisplayContext* context, hkgVertexShaderDX9* currentVertexProgram, hkgPixelShaderDX9* currentFragmentProgram) const;
		void renderParticlesPointSprites( hkgDisplayContext* context, hkgVertexShaderDX9* currentVertexProgram, hkgPixelShaderDX9* currentFragmentProgram ) const;
		void renderParticlesQuads( hkgDisplayContext* context, hkgVertexShaderDX9* currentVertexProgram, hkgPixelShaderDX9* currentFragmentProgram ) const;

		LPDIRECT3DDEVICE9 m_device;

		LPDIRECT3DVERTEXDECLARATION9 m_posTexDecl;
		LPDIRECT3DVERTEXDECLARATION9 m_posFVFDecl;
...
#endif // HK_GRAPHICS_PARTICLE_DISPLAY_OBJECT_DX9__H
