...
#include <Graphics/Ogls/Pc/Window/hkgWindowPCOGLS.h>
...
class hkgWindowManagerPCOGLS : public hkReferencedObject, public hkSingleton<hkgWindowManagerPCOGLS>
{
public:
	...
	bool addWindow( HWND handle, hkgWindowPCOGLS* w );
	bool setHWND( HWND handle, hkgWindowPCOGLS* w );

	hkgWindowPCOGLS* findWindow( HWND handle ) const;
	
	bool removeWindow( HWND handle );
	
protected:

	hkPointerMap<void*, hkgWindowPCOGLS*> m_hwndMap; 
	HANDLE m_hMutex;
};
