...
#ifndef HK_GRAPHICS_PS3_TEXTURE_H
#define HK_GRAPHICS_PS3_TEXTURE_H

#include <Graphics/Common/Texture/hkgTexture.h>

class hkgTextureOGLS : public hkgTexture
{
public:

	virtual bool realize(bool dynamic, HKG_TEXTURE_USAGE_HINT useHint = HKG_TEXTURE_USAGE_UNKOWN );	
	virtual void free();		

	static hkgTexture* createTextureOGLS(hkgDisplayContext* c) { return new hkgTextureOGLS(c); }
...
protected:
	...
	inline hkgTextureOGLS(hkgDisplayContext* context);
	virtual ~hkgTextureOGLS();
	
	unsigned int m_glID;
};

#include <Graphics/Ogls/Shared/Texture/hkgTextureOGLS.inl>

#endif // HK_GRAPHICS_PS3_TEXTURE_H
