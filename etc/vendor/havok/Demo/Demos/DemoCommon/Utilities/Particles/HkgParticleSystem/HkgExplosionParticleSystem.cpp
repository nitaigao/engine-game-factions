...
HkgExplosionParticleSystem::HkgEmitParams::HkgEmitParams()
	: m_minSpriteIndex(0),
	  m_maxSpriteIndex(0)
{
}

HkgExplosionParticleSystem::HkgExplosionParticleSystem( int randomSeed, hkDemoEnvironment* env,
	hkgMaterial* material, int maxNumParticlesDrawn, int particleStride )
	: ExplosionParticleSystem(randomSeed, particleStride),
	  m_depthSortEnabled(true),
...
HkgExplosionParticleSystem::~HkgExplosionParticleSystem()
{
	m_demoEnvironment->m_window->getContext()->lock();
...
void HkgExplosionParticleSystem::clear()
{
...
void HkgExplosionParticleSystem::draw()
{
	m_demoEnvironment->m_window->getContext()->lock();
...
void HkgExplosionParticleSystem::initParticles(int numParticles, void* particleData, hkReal timestep, const void* emitParams)
{
	ExplosionParticleSystem::initParticles(numParticles, particleData, timestep, emitParams);

	HK_ASSERT(0x7ef19315, emitParams);
