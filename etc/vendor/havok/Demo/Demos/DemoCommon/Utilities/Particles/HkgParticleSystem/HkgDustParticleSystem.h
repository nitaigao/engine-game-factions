...
#ifndef HK_HKG_DUST_PARTICLE_SYSTEM_H
#define HK_HKG_DUST_PARTICLE_SYSTEM_H
...
// A dust particle system that uses HKG for rendering.
// Note that this class imposes a cap on the number of particles, even though DustParticleSystem does not.
// Depth-sorting is optional and is disabled by default.
class HkgDustParticleSystem : public DustParticleSystem
{
	public:

		HkgDustParticleSystem( int randomSeed, hkDemoEnvironment* env, hkgMaterial* material,
			int maxNumParticlesDrawn, int particleStride = sizeof(HkgParticle) );

		virtual ~HkgDustParticleSystem();
...
#endif	// HK_HKG_DUST_PARTICLE_SYSTEM_H
