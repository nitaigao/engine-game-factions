...
void SixWheelTruckSetup::buildVehicle( const hkpWorld* world, hkpVehicleInstance& vehicle )
{
	//
...
void SixWheelTruckSetup::setupVehicleData( const hkpWorld* world, hkpVehicleData& data )
{
	...
	data.m_wheelParams[4].m_friction = 1.27f;
	data.m_wheelParams[5].m_friction = 1.27f;
	...
	for ( int i = 0 ; i < data.m_numWheels ; i++ )
	{
		...
		data.m_wheelParams[i].m_forceFeedbackMultiplier = 1.0f;
		data.m_wheelParams[i].m_maxContactBodyAcceleration = hkReal(data.m_gravity.length3()) * 2;
	}
}


void SixWheelTruckSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultAnalogDriverInput& driverInput )
{
...
void SixWheelTruckSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultSteering& steering )
{   
	...
	// [mph/h].
	steering.m_maxSpeedFullSteeringAngle = 130.0f * (1.605f / 3.6f); // [mph/h]
...
void SixWheelTruckSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultEngine& engine )
{
	engine.m_maxTorque = 500.0f;

	engine.m_minRPM = 1000.0f;
	engine.m_optRPM = 5000.0f;

	// This value is also used to calculate the m_primaryTransmissionRatio.
	engine.m_maxRPM = 8000.0f;

	engine.m_torqueFactorAtMinRPM = 0.80f;
	engine.m_torqueFactorAtMaxRPM = 0.80f;

	engine.m_resistanceFactorAtMinRPM = 0.05f;
	engine.m_resistanceFactorAtOptRPM = 0.10f;
	engine.m_resistanceFactorAtMaxRPM = 0.30f;
}

void SixWheelTruckSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultTransmission& transmission )
{
	int numGears = 4;
	...
	transmission.m_gearsRatio[0] = 3.1f;
	...
	transmission.m_wheelsTorqueRatio[2] = 0.2f;
	transmission.m_wheelsTorqueRatio[3] = 0.2f;
	transmission.m_wheelsTorqueRatio[4] = 0.2f;
	transmission.m_wheelsTorqueRatio[5] = 0.2f;

	const hkReal vehicleTopSpeed = 75.0f; 	 
	const hkReal wheelRadius = 0.5f; 	 
	const hkReal maxEngineRpm = 8000.0f; 	 
...
void SixWheelTruckSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultBrake& brake )
{
	...
	brake.m_wheelBrakingProperties[3].m_isConnectedToHandbrake = false;
...
void SixWheelTruckSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultSuspension& suspension ) 
{
	suspension.m_wheelParams.setSize( data.m_numWheels );
	suspension.m_wheelSpringParams.setSize( data.m_numWheels );

	hkReal wheelsLength = 0.35f;
	...
	suspension.m_wheelSpringParams[4].m_strength = str;
	suspension.m_wheelSpringParams[5].m_strength = str;
	...
	//
	{
		...
		const hkReal hardPointMidX = 0.0f;
		const hkReal hardPointBackX = -1.1f;
		const hkReal hardPointY = -0.2f;
		const hkReal hardPointZ = 0.95f;
		...
		suspension.m_wheelParams[4].m_hardpointChassisSpace.set ( hardPointMidX, hardPointY, -hardPointZ); 
		suspension.m_wheelParams[5].m_hardpointChassisSpace.set ( hardPointMidX, hardPointY, hardPointZ); 
	}
...
void SixWheelTruckSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultAerodynamics& aerodynamics ) 
{
...
void SixWheelTruckSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultVelocityDamper& velocityDamper )
{
...
void SixWheelTruckSetup::setupTyremarks( const hkpVehicleData& data, hkpTyremarksInfo& tyreMarks ) 
{
