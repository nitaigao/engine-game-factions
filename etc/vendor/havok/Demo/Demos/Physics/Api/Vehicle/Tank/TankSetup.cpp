...
void TankSetup::setupVehicleData( const hkpWorld* world, hkpVehicleData& data )
{
	...
	for ( int i = 0 ; i < data.m_numWheels ; i++ )
	{
		...
		data.m_wheelParams[i].m_width = 0.4f;
		data.m_wheelParams[i].m_mass = 100.0f;

		data.m_wheelParams[i].m_viscosityFriction = 0.07f;
		data.m_wheelParams[i].m_slipAngle = 0.0f;
		data.m_wheelParams[i].m_maxFriction = 2.0f * data.m_wheelParams[i].m_friction;
		data.m_wheelParams[i].m_forceFeedbackMultiplier = 3.0f;
		data.m_wheelParams[i].m_maxContactBodyAcceleration = hkReal(data.m_gravity.length3()) * 5;
	}
}


void TankSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultAnalogDriverInput& driverInput )
{
...
void TankSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultSteering& steering )
{   
	steering.m_doesWheelSteer.setSize( data.m_numWheels );

	// degrees
	steering.m_maxSteeringAngle = HK_REAL_PI / 2.0f;
	// [mph/h].
	steering.m_maxSpeedFullSteeringAngle = 10.0f;

	// All 4 wheels steer.  TankSteering turns the back wheels in the reverse direction.
...
void TankSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultEngine& engine )
{
...
void TankSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultTransmission& transmission )
{
	int numGears = 4;
	...
	// Increase this to make the vehicle reverse more quickly.
	transmission.m_reverseGearRatio = 1.20f;
	...
	// Tank engine pushes equally hard with front and back wheels i.e. the entire tank tread
	// each side.
	...
	// US M1 Abrams approximate top speed 45 mph. British Challenger 2 approximate top speed 37 mph.
	const hkReal vehicleTopSpeed = 45.0f; 	 
...
void TankSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultBrake& brake )
{
	...
	// Can brake with all wheels i.e. all parts of the tank treads.
	brake.m_wheelBrakingProperties[0].m_isConnectedToHandbrake = true;
...
void TankSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultSuspension& suspension ) 
{
	...
	//
	{
		...
		suspension.m_wheelParams[2].m_hardpointChassisSpace.set ( -hardPointX, hardPointY, -hardPointZ); 
		suspension.m_wheelParams[3].m_hardpointChassisSpace.set ( -hardPointX, hardPointY,  hardPointZ); 
	}
...
void TankSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultAerodynamics& aerodynamics ) 
{
...
void TankSetup::setupComponent( const hkpVehicleData& data, hkpVehicleDefaultVelocityDamper& velocityDamper )
{
