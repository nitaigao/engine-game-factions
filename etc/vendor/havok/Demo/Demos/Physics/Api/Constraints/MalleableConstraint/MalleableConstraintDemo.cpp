...
#include <Demos/Physics/Api/Constraints/MalleableConstraint/MalleableConstraintDemo.h>
...
MalleableConstraintDemo::MalleableConstraintDemo(hkDemoEnvironment* env)
:	hkDefaultPhysicsDemo(env)
{
	...
	// 
	{
		...
		// Create malleable constraint wrapper
		hkpMalleableConstraintData* bender = new hkpMalleableConstraintData( axle );
		bender->setStrength( 0.01f );

		// Remove reference to "axle" since the BreakableConstraint now owns it.
		axle->removeReference();

		// Add the malleable constraint to the world
		hkpConstraintInstance* constraint = new hkpConstraintInstance(moveableBody, fixedBody, bender);
		m_world->addConstraint( constraint); 	

		constraint->removeReference();
		bender->removeReference();
	}
...
"This demo shows how to use the malleable constraint wrapper " \
...
"exceeded and the constraint bends.";

HK_DECLARE_DEMO(MalleableConstraintDemo, HK_DEMO_TYPE_PRIME, "A single malleable constraint wrapping a hinge", helpString);
