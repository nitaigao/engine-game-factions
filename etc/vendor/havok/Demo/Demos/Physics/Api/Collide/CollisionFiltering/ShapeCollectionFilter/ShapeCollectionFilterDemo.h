...
// This selection of triangles is archived by adding material
// information to the mesh, which includes also collision filter
// data.

#ifndef HK_ShapeCollectionFilterDemo_H
#define HK_ShapeCollectionFilterDemo_H
...
class ShapeCollectionFilterDemo: public hkDefaultPhysicsDemo
{
	public:

		HK_DECLARE_CLASS_ALLOCATOR(HK_MEMORY_CLASS_DEMO);

		ShapeCollectionFilterDemo(hkDemoEnvironment* env);
	...
	protected:
		...
		// 
		enum { NUM_VERTICES = 32, NUM_MATERIALS = 4 };
		...
		hkpMeshMaterial m_materials[NUM_MATERIALS];

		void createMoveableListShape(hkVector4& position, hkVector4& velocity, int colorIdx);
};



#endif	// HK_ShapeCollectionFilterDemo_H
