...
class WindRegionDemo : public hkDefaultPhysicsDemo
{
		hkpWindRegion* m_region;
		hkVector4 initialPosition( hkReal height, int i, int gridWidth, hkReal gap );
		hkpRigidBody* createCompositeBody( hkReal width, hkReal mass, const hkVector4& position );
	public:
		WindRegionDemo( hkDemoEnvironment* env );
		virtual ~WindRegionDemo();
};
