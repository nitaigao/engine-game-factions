...
#include <Demos/Physics/Api/Dynamics/RigidBodies/DisablingConstraints/DisablingConstraintsDemo.h>
...
hkDemo::Result DisablingConstraintsDemo::stepDemo()
{
	m_time += m_timestep;
	...
	if (m_time > 1.0f)
	{
		...
		hkpPhysicsSystem* sys = m_world->getWorldAsOneSystem();
		{
			for (int c = 0; c < sys->getConstraints().getSize(); c++)
			{
				hkpConstraintInstance* constraint = sys->getConstraints()[c];
				constraint->setEnabled(m_constraintsEnabled);
			}
		}
		sys->removeReference();
...
HK_DECLARE_DEMO(DisablingConstraintsDemo, HK_DEMO_TYPE_PRIME, "A single Limited Hinge constraint, with viewer", helpString);
