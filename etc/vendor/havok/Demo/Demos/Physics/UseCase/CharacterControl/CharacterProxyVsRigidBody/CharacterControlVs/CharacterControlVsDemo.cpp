...
#include <Demos/Physics/UseCase/CharacterControl/CharacterProxyVsRigidBody/CharacterControlVs/CharacterControlVsDemo.h>
...
CharacterControlVsDemo::CharacterControlVsDemo(hkDemoEnvironment* env)
:	hkDefaultPhysicsDemo(env)
{
		...
		if (physics)
		{
			...
			for (int i=0; i<psys.getSize(); i++)
			{
				...
				for (int rb=0; rb < system->getRigidBodies().getSize(); rb++)
				{
					...
					body->setCollisionFilterInfo(hkpGroupFilter::calcFilterInfo( CharacterControlVsDemo::LAYER_LANDSCAPE,0 ));
	...
	//	Create a character proxy object
	{
		...
		m_phantom = new hkpSimpleShapePhantom( m_standShape, hkTransform::getIdentity(), hkpGroupFilter::calcFilterInfo( CharacterControlVsDemo::LAYER_CHARACTER_PROXY,0 ) );
	...
	//	Create a character rigid body object
	{
		...
		info.m_collisionFilterInfo = hkpGroupFilter::calcFilterInfo( CharacterControlVsDemo::LAYER_CHARACTER_RIGIDBODY,0 );
...
CharacterControlVsDemo::~CharacterControlVsDemo()
{
...
hkDemo::Result CharacterControlVsDemo::stepDemo()
{
	...
	// Display character states
	{
		...
		char buffer[255];
		hkString::snprintf(buffer, 255, "Proxy character controller state: %s\nRigid body character controller state: %s", stateStrCC, stateStrRBC);
...
void CharacterControlVsDemo::doCharacterProxy()
{
	...
	hkpCharacterOutput output;
	{
		...
		if ( m_listenerProxy->m_atLadder && ( ( input.m_inputUD < 0 ) || ( input.m_surfaceInfo.m_supportedState != hkpSurfaceInfo::SUPPORTED ) ) )
		{
			...
			input.m_surfaceInfo.m_surfaceNormal = m_listenerProxy->m_ladderNorm;
			input.m_surfaceInfo.m_surfaceVelocity = m_listenerProxy->m_ladderVelocity;
		}
	}
...
void CharacterControlVsDemo::doCharacterRigidBody()
{
...
void CharacterControlVsDemo::cameraHandling()
{
...
void CharacterControlVsDemo::swapPhantomShape( hkpShape* newShape )
{
...
HK_DECLARE_DEMO(CharacterControlVsDemo, HK_DEMO_TYPE_PRIME, "RigidBody/Proxy Character Test", helpString);
