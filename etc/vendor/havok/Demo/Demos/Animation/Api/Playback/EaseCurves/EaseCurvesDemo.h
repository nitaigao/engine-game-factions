...
#ifndef EASECURVESDEMO_H
#define EASECURVESDEMO_H

#include <Demos/DemoCommon/DemoFramework/hkDefaultAnimationDemo.h>

#define NUM_ANIMS 3

class EaseCurvesDemo : public hkDefaultAnimationDemo
{
	public:

		HK_DECLARE_CLASS_ALLOCATOR( HK_MEMORY_CLASS_DEMO );

		EaseCurvesDemo( hkDemoEnvironment* env );
	...
	private:
		...
		class hkaAnimatedSkeleton* m_skeletonInstance;
			...
			// The texture for the graph image
		class hkgTexture*			m_texture;
		class hkgDisplayContext*	m_context;
		unsigned char*		m_graphCanvasData;
		hkBool				m_textureRealized;
		void drawTexture( hkgTexture* texture );
};

#endif // EASECURVESDEMO_H
