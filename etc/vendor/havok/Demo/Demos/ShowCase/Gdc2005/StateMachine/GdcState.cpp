...
GdcAnimationMachine::GdcAnimationMachine( class hkaAnimatedSkeleton* animatedSkeleton )
:  m_currentTime(0.0f), m_animatedSkeleton( animatedSkeleton )
{
	m_animatedSkeleton->addReference();
}

GdcAnimationMachine::~GdcAnimationMachine( )
{
	m_animatedSkeleton->removeReference();
}

void GdcAnimationMachine::addCommand( const AnimationCommand& command )
{
	// Set the global time for the command
	GdcAnimationMachine::AnimationCommand gCmd = command;
...
void GdcAnimationMachine::process( const AnimationCommand& command )
{
...
void GdcAnimationMachine::update( hkReal timestep )
{
...
void GdcAnimationMachine::flushQueue()
{
	m_eventQueue.clear();
}

hkReal GdcAnimationMachine::getCurrentTime() const
{
	return m_currentTime;
}

void GdcAnimationMachine::resetTime(hkReal newTime)
{
...
// State Manager
GdcStateManager::~GdcStateManager()
{
	hkMap<hkUint32, GdcState*>::iterator itr = m_registeredStates.begin();
...
void GdcStateManager::registerState( hkUint32 stateId, GdcState* state )
{
	state->addReference();
...
GdcState* GdcStateManager::getState( hkUint32 stateId ) const
{
	return m_registeredStates.get(stateId, HK_NULL);
}

// State machine
GdcStateContext::GdcStateContext(const GdcStateManager* manager)
	: m_stateManager(manager), m_currentStateId(0xffffffff), m_currentTime(0.0f), m_lastStateChange(0.0f)
{
	m_stateManager->addReference();
}

GdcStateContext::~GdcStateContext()
{
	m_stateManager->removeReference();
}

hkUint32 GdcStateContext::getCurrentState() const
{
	return m_currentStateId;
}

void GdcStateContext::setCurrentState(hkUint32 state, GdcAnimationMachine* machine )
{
	if (m_currentStateId != state)
	{
		...
		GdcState* oldState = m_stateManager->getState( oldStateId );
		if (oldState)
		{
			oldState->leaveState( state, machine );
		}

		GdcState* newState = m_stateManager->getState( state );
		if (newState)
		{
...
void GdcStateContext::update(hkReal timestep, const GdcStateInput* input )
{
	GdcState* currentState = m_stateManager->getState( m_currentStateId );
	if (currentState)
	{
...
// Time control
hkReal GdcStateContext::getCurrentTime() const
{
	return m_currentTime;
}

// Time control
hkReal GdcStateContext::getLastStateChange() const
{
